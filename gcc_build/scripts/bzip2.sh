...
function download_bzip2_src() {
    ...
    if [ ! -f "${BUILD_DIR}"/bzip2/src/bzip2-${BZIP2_VER}.tar.gz ]; then
        ...
        popd > /dev/null # "${BUILD_DIR}"/bzip2/src
        echo 'done'
    fi
...
function prepare_bzip2() {
    ...
    if [ ! -f "${BUILD_DIR}"/bzip2/src/bzip2-${BZIP2_VER}/patched_02.marker ]; then
        ...
        patch -p1 -i "${PATCHES_DIR}"/bzip2/0002-bzip2-buildsystem.all.patch \
            >> "${LOGS_DIR}"/bzip2/bzip2_patch.log 2>&1 || exit 1
    ...
    popd > /dev/null # "${BUILD_DIR}"/bzip2/src/bzip2-$BZIP2_VER
    echo 'done'
...
function build_bzip2() {
    ...
    do
        ...
        cp -af "${PREIN_DIR}"/bzip2/mingw$_bitval "${DST_DIR}"
        echo 'done'
    done
