...
declare -r LOGS_DIR=${HOME}/logs/x264
if [ ! -d $LOGS_DIR ] ; then
    mkdir -p $LOGS_DIR
fi
...
function build_x264() {
    ...
    local -r SAMPLE_FILES="${HOME}/y4m/deadline_cif.y4m ${HOME}/y4m/foreman_cif.y4m ${HOME}/y4m/soccer_4cif.y4m"
    local -ri patches_num=23
    ...
    do
        ...
        do
            ...
            printf "===> making fprofiled x264 %s\n" "${_target}"
            make -j9 -O fprofiled VIDS="${SAMPLE_FILES}" \
                > ${LOGS_DIR}/x264_make_${bitdepth}bit_${buildtype}.log 2>&1 || exit 1
            echo "done"

            if [ "${bitdepth}" -eq 8 -a "${buildtype}" = "all" ] ; then
                local X264_REVISION=$(x264 --version | grep "x264" | awk '{print $2}' \
            ...
            if [ "${bitdepth}" -eq 8 ] ; then
                if [ "${buildtype}" = "all" ] ; then
                    ...
                    ./x264.exe --fullhelp > ${DEST_DIR}/lite/x264_x64_lite_help.txt
                fi
            else
                if [ "${buildtype}" = "all" ] ; then
                    ...
                    ./x264.exe --fullhelp > ${DEST_DIR}/lite/x264_x64_lite-10bit_help.txt
                fi
            fi
            echo "done"
    ...
    tar cf - x264_r${X264_REVISION}_patches | lzip -9 -o \
