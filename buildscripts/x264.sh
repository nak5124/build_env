    ...
    do
        ...
        do
            if [ "$buildtype" == "all" ] ; then
                ...
                local _format="420"
                local _opt="--disable-interlaced --disable-swscale --disable-lavf"
                local _clopt="--disable-opencl"
                if [ "$bitdepth" -eq 8 ] ; then
            ...
            make -j9 -O fprofiled VIDS="$SAMPLE_FILES" \
                > ${LOGS_DIR}/x264_make_${bitdepth}bit_${buildtype}.log 2>&1 || exit 1
            echo "done"

            if [ "$bitdepth" -eq 8 ] && [ "$buildtype" == "all" ] ; then
                local X264_REVISION=`x264 --version|grep "x264"|awk '{print $2}'|awk -F . '{print $3}'|awk -F + '{print $1}'`
                local DEST_DIR=/usr/local/x264/x264_r${X264_REVISION}_x64_v2
            ...
            if [ "$bitdepth" -eq 8 ] ; then
                if [ "$buildtype" == "all" ] ; then
                    cp -fa ${HOME}/x264/x264.exe ${DEST_DIR}/x264_x64.exe
                    ${HOME}/x264/x264.exe --fullhelp > ${DEST_DIR}/x264_x64_help.txt
                else
                    cp -fa ${HOME}/x264/x264.exe ${DEST_DIR}/lite/x264_x64_lite.exe
                    ${HOME}/x264/x264.exe --fullhelp > ${DEST_DIR}/lite/x264_x64_lite_help.txt
                fi
            else
                if [ "$buildtype" == "all" ] ; then
                    cp -fa ${HOME}/x264/x264.exe ${DEST_DIR}/x264_x64-10bit.exe
                    ${HOME}/x264/x264.exe --fullhelp > ${DEST_DIR}/x264_x64-10bit_help.txt
                else
                    cp -fa ${HOME}/x264/x264.exe ${DEST_DIR}/lite/x264_x64_lite-10bit.exe
                    ${HOME}/x264/x264.exe --fullhelp > ${DEST_DIR}/lite/x264_x64_lite-10bit_help.txt
                fi
            fi
            echo "done"
